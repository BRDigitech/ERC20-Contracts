...
contract ContentDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Content_Dividend_Tracker", "Content_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Content_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Content_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Content contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Content_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Content_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CONTENTCOIN is BEP20, Ownable, SafeToken, LockToken {
    ...
    ContentDividendTracker public dividendTracker;
...
Token Name:  CONTENT COIN 2.0
Token Symbol:  CONT 
5% marketing fee  in bnb 
2% back to holders in bnb 
2% added to the liquidity pool in BNB 
*/
    constructor() BEP20("CONTENT COIN 2.0", "CONT") {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 5;
        marketingWallet = payable(0x895c160D5c1988d0e92556B1E34aEC818Dc5fB86); // please provide marketing address ok one second
     //   address pinkAntiBot_ ;
    	...
    	dividendTracker = new ContentDividendTracker();
 ...
 function setEnableAntiBot(bool _enable) external onlyOwner {
   antiBotEnabled = _enable;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Content: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Content: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Content: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Content: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Content: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Content: Cannot update gasForProcessing to same value");
