...
contract CBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("CBNB_Dividend_Tracker", "CBNB_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "CBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "CBNB_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CBNB is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    CBNBDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1000000000 * (10**18); 
    uint256 public swapTokensAtAmount = 1000000 * (10**18); // 0.1%
    ...
    constructor() ERC20("CRYSTALBNB", "CBNB") {
        BNBRewardsFee = 5;
        liquidityFee = 3;
        extraFeeOnSell = 4; // extra fee on sell
        marketingFee = 2;
        marketingWallet = payable(0xF9761AF66F2cC462E7d9F33489B180059Bf1551b);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new CBNBDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CBNB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CBNB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CBNB: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CBNB: Cannot update gasForProcessing to same value");
