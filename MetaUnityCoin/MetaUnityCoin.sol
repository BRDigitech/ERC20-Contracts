...
contract MUCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("MUC_Dividend_Tracker", "MUC_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MUC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MUC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MUC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MUC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "MUC_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MUC is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    MUCDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _burnFee) public onlyOwner {
        ...
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(burnFee).add(burnFee); // total fee transfer and buy
    }
    ...
    constructor() ERC20("Meta Unity Coin", "MUC") {
        BNBRewardsFee = 2;
        liquidityFee = 3;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 5;
        burnFee = 2;
        marketingWallet = payable(0x958D53AbFA2aE45F7C938E59Acf42F9329BeE1EA);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(burnFee); // total fee transfer and buy

    	dividendTracker = new MUCDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MUC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MUC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MUC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MUC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MUC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MUC: Cannot update gasForProcessing to same value");
