//https://testnet.bscscan.com/token/0xd16a0e68b34cdf09952c7ced25c6463c2fe8d9a0#code

/**
 *Submitted for verification at testnet.bscscan.com on 2021-12-01
*/
...
contract DVERSEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("DVERSE_Dividend_Tracker", "DVERSE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DVERSE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DVERSE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DVERSE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DVERSE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "DVERSE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DefVerse is ERC20, Ownable, SafeToken, LockToken {
    ...
    DVERSEDividendTracker public dividendTracker;
...
BUY 5% tax - BNB (in a Wallet) Required wallet address
Sell 10% tax - BNB (in a Wallet) Required wallet address

TOTAL SUPPLY - 500m 500,000,000
Mintable : Yes
burnable : Yes
...
BurnFrom
*/
    constructor() ERC20("DefVerse", "DVERSE") {
        BNBRewardsFee = 0;
        liquidityFee = 0;
        extraFeeOnSell = 5; // extra fee on sell
        marketingFee = 5;
        marketingWallet = payable(0xa2c4b59B66B61670c87C51510f4e9bEaE82936ef);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new DVERSEDividendTracker();
   ...
   function burn(address account, uint256 amount)  public onlyOwner{
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DVERSE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DVERSE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DVERSE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DVERSE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DVERSE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DVERSE: Cannot update gasForProcessing to same value");
