...
// https://bscscan.com/token/0x18BCaEA0c1d71Cbdd9063e5c45BF374c57F2749b#code
/**
...
contract LUVEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("LUVE_Dividend_Tracker", "LUVE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "LUVE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "LUVE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LUVE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LUVE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "LUVE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Concierge is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    LUVEDividendTracker public dividendTracker;
    ...
    function addBotToBlackList(address account) external onlyOwner() {
        require(account != 0x10ED43C718714eb63d5aA57B78B54704E256024E, 'We can not blacklist pancake swap router.');
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _TeamFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        TeamFee = _TeamFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(TeamFee); // total fee transfer and buy
    }
    ...
    function setTeamWallet(address payable _newTeamWallet) public onlyOwner {
        TeamWallet = _newTeamWallet;
    }
    constructor() ERC20("Concierge", "CNG") {
        BNBRewardsFee = 3;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        TeamFee = 5;
        TeamWallet = payable(0xfDa625b8f17429EBf2c7bC2f05bF91D32055253e);

        totalFees = BNBRewardsFee.add(liquidityFee).add(TeamFee); // total fee transfer and buy

    	dividendTracker = new LUVEDividendTracker();
   ...
   function mint(address account, uint256 amount)  public onlyOwner{
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LUVE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LUVE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LUVE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LUVE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LUVE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LUVE: Cannot update gasForProcessing to same value");
