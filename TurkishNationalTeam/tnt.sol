...
contract TURK is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 192300000 * 10**8;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Turkish National Team";
    string private constant _symbol = "TURK";
    ...
    uint256 public numTokensSellToAddToLiquidity = 2e4 * 10**8;
    ...
    event SetDevWalletAddress(address _address);
    ...
    constructor(address router, address devWallet, address liqWallet) {
    ...
    function setBuyTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        require(_buyTaxFee + _buyDevFee + _buyLiquidityFee <= 100, "Total fees exceed 10%");
    }
    ...
    function setSellTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        require(_sellTaxFee + _sellDevFee + _sellLiquidityFee <= 100, "Total fees exceed 10%");
    }
    ...
    function setDevWalletAddress(address _address) external onlyOwner() {
        ...
        emit SetDevWalletAddress(_address);
    }
