...
contract TeddyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Teddy_Dividend_Tracker", "Teddy_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Teddy_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Teddy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Teddy contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Teddy_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Teddy_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TeddyToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    TeddyDividendTracker public dividendTracker;
...
Symbol: Teddy
Decimals 18
Owners Address  0x135fc9E413bDF47dc92a7dA0cC0D57353F359794
Team wallet  0x857374f541aA9FF52b455AB3f6C928933DEA6Db8
100B total supply 100,000,000,000
buy/sell tax 10%
2%liquidity
4%rewards (bnb)
4% team wallet (bnb)
*/
    constructor() ERC20("Teddy", "Teddy") {
        BNBRewardsFee = 4;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        TeamFee = 4;
        TeamWallet = payable(0x857374f541aA9FF52b455AB3f6C928933DEA6Db8);

        totalFees = BNBRewardsFee.add(liquidityFee).add(TeamFee); // total fee transfer and buy

    	dividendTracker = new TeddyDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Teddy: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Teddy: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Teddy: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Teddy: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Teddy: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Teddy: Cannot update gasForProcessing to same value");
