...
contract StarDustDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("StarDust_Dividend_Tracker", "StarDust_Dividend_Tracker") {
        claimWait = 14400;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "StarDust_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "StarDust_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main StarDust contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "StarDust_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "StarDust_Dividend_Tracker: Cannot update claimWait to same value");
...
contract StarDust is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    StarDustDividendTracker public dividendTracker;
    ...
    constructor() ERC20("StarDust", "SD") {
        BNBRewardsFee = 0;
        liquidityFee = 5;
        extraFeeOnSell = 8; // extra fee on sell
        MarketingFee = 3;
        MarketingWallet = payable(0x9bcBE4Af33C43E12c82d2fcD77DF6AF7F82174A2); // Acc1

        totalFees = BNBRewardsFee.add(liquidityFee).add(MarketingFee); // total fee transfer and buy

    	dividendTracker = new StarDustDividendTracker();
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "StarDust: Automated market maker pair is already set to that value");
