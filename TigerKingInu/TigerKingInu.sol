/**
 *Submitted for verification at testnet.bscscan.com on 2021-11-22
*/
...
contract TIKINUDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("TIKINU_Dividend_Tracker", "TIKINU_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TIKINU_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TIKINU_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TIKINU contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TIKINU_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "TIKINU_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TIKINU is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    TIKINUDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Tiger King Inu", "TIKINU") {
        BNBRewardsFee = 4;
        liquidityFee = 4;
        extraFeeOnSell = 1; // extra fee on sell
        marketingFee = 4;
        marketingWallet = payable(0x746823B623d98756EB2e62eC40966157b0AC2Fa2);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new TIKINUDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TIKINU: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TIKINU: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TIKINU: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TIKINU: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TIKINU: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TIKINU: Cannot update gasForProcessing to same value");
