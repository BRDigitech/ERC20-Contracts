...
contract DVTNDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("DVTN_Dividend_Tracker", "DVTN_Dividend_Tracker") {
        claimWait = 86400;
        minimumTokenBalanceForDividends = 100000000 * (10**9); //must hold 100000000+ tokens
    }
    
    function _UpdateMinimumTokenBalanceForDividends(uint256 NewminiMumTokenBalanceForDividends) public onlyOwner {
        minimumTokenBalanceForDividends = NewminiMumTokenBalanceForDividends;
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DVTN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DVTN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DVTN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DVTN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "DVTN_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DuvalToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    DVTNDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 100000 * (10**18);
    uint256 public swapTokensAtAmount = 2 * 10**2 * (10**18);
    ...
    function setFee(uint256 _maticRewardFee, uint256 _liquidityFee, uint256 _TreasuryFee) public onlyOwner {
        MATICRewardsFee = _maticRewardFee;
        liquidityFee = _liquidityFee;
        TreasuryFee = _TreasuryFee;

        totalFees = MATICRewardsFee.add(liquidityFee).add(TreasuryFee); // total fee transfer and buy
    }
    ...
    function setTreasuryWallet(address payable _newTreasuryWallet) public onlyOwner {
        TreasuryWallet = _newTreasuryWallet;
    }

    constructor() ERC20("Duval Token", "DVTN") {
        MATICRewardsFee = 0;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        TreasuryFee = 3;
        TreasuryWallet = payable(0xCc9aF8cfDa1eC7e19B4D17ae5e4Cbdf4bb6520F5);

        totalFees = MATICRewardsFee.add(liquidityFee).add(TreasuryFee); // total fee transfer and buy

    	dividendTracker = new DVTNDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DVTN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DVTN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DVTN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DVTN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DVTN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DVTN: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // worthy Treasury fee
        uint256 TreasuryAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(TreasuryFee);
    ...
    function swapAndSendBNBToTreasury(uint256 tokenAmount) private {
        swapTokensForEth(tokenAmount, TreasuryWallet);
    }
