...
contract FundTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("FundToken_Dividend_Tracker", "FundToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "FundToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "FundToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FundToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FundToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "FundToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FundToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    FundTokenDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _charityFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        charityFee = _charityFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(charityFee); // total fee transfer and buy
    }
    ...
    function setcharityWallet(address payable _newBuybakWallet) public onlyOwner {
        charityWallet = _newBuybakWallet;
    }

    constructor() ERC20("FundToken", "FDT") {
        BNBRewardsFee = 10;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 3;
        charityFee = 3;
        marketingWallet = payable(0x06763e67B86409CB2f136a8775c8320FdE04a401);
        charityWallet = payable(0x6BC4CF4e4b3F203C01842E7D5A8cD4aD9Ca7cC4C);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(charityFee); // total fee transfer and buy

    	dividendTracker = new FundTokenDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FundToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FundToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FundToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FundToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FundToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FundToken: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(charityFee)).mul(marketingFee);
        marketingWallet.transfer(marketingAmount);
        
         uint256 buybackAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(marketingFee)).mul(charityFee);
        charityWallet.transfer(buybackAmount);
    ...
    function swapAndSendBNBToBuybak(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, charityWallet);
    }
