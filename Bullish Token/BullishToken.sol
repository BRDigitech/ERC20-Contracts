...
contract BLTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BLT_Dividend_Tracker", "BLT_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BLT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BLT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BLT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BLT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BLT_Dividend_Tracker: Cannot update claimWait to same value");
...
2% auto-added to the liquidity pool
5% transferred to marketing wallet in BNBs(I need marketing wallet address)
2% reflection/ distribution to holder in BNBs

*/
contract BullishToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BLTDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 50000000000 * (10**18);
    ...
    constructor() ERC20("Bullish Token", "BLT") {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 5;
        marketingWallet = payable(0xAB50Ed9846A4cB070B623967406540d52575e870);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new BLTDividendTracker();
        ...
        */
        _mint(owner(), 5000000000000 * (10**18));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BLT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BLT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BLT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BLT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BLT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BLT: Cannot update gasForProcessing to same value");
